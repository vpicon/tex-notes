...
\end{enumerate}
    ...
    We can use the substitution method to establish either upper or lower bounds on a recurrence. As an example consider the recurrence given by: $T(n) = 2T(\lfloor n/2 \rfloor) + n$. We guess that the solution is $T(n) = O(n\log{n})$. So, we claim that $T(n) \leq cn\log{n}$, for some $c > 0$. Proving the claim would then imply that $T(n) = O(n\log{n})$.
